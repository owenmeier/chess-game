...
function createPiece(name, color) {
	const piece = { name, color, hasMoved: false };
	return piece;
}
...
export default function Board({
	...
	function executeMove(selected, row, col) {
		...
		const isCastling =
			selected.piece.name == PIECES.KING && Math.abs(selected.col - col) == 2;

		if (isCastling) {
			const isKingSide = col > selected.col;
			const rookCol = isKingSide ? 7 : 0;
			const newRookCol = isKingSide ? col - 1 : col + 1;

			const rook = newBoard[row][rookCol];
			newBoard[row][newRookCol] = { ...rook, hasMoved: true };
